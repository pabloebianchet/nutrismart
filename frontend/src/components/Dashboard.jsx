...
const Dashboard = () => {
  ...
  const lastHistoryUserRef = useRef(null);
  ...
  useEffect(() => {
    if (!user?.googleId) {
      lastHistoryUserRef.current = null;
      setHistory([]);
      setLoading(false);
      return;
    }
    ...
    if (lastHistoryUserRef.current === user.googleId) {
      setLoading(false);
      return;
    }
    ...
    const fetchHistory = async () => {
      setLoading(true);
      lastHistoryUserRef.current = user.googleId;
