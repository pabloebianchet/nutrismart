...
const Placeholder = ({ w, h, state }) => (
  <Box sx={{ width: w, height: h, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "flex-end", pb: 1.5, gap: 0.5 }}>
    <Typography sx={{ fontSize: h * 0.32, lineHeight: 1 }}>{EMOJIS[state - 1]}</Typography>
    ...
    >
      ...
      {imgOk ? (
        <Box
          ...
          onError={() => { setImgOk(false); setLoaded(true); }}
          sx={{
            width:  "100%",
            height: "100%",
            objectFit:      "contain",
      ...
      {showLabel && (
        <Typography sx={{ position: "absolute", bottom: -20, left: "50%", transform: "translateX(-50%)", fontSize: 10, fontWeight: 700, color: "rgba(255,255,255,0.5)", whiteSpace: "nowrap" }}>
          Nivel {state} / 5
        </Typography>
      )}
    </Box>
